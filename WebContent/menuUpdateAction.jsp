...
<jsp:setProperty property="menuNumber" name="menu" />
...
<body>
	...
	if (userID == null) {
		...
		try {
			...
			menuNumber = Integer.parseInt( multi.getParameter("menuNumber") );
		...
		if ("".equals(menuTitle) || "".equals(menuContent)) {
			...
			MenuImageDAO menuImageDAO = new MenuImageDAO();
				...
				if (menuImageNumber == 0) {
					...
					if (menuDAO.updateImage(menuTitle, menuContent,0, menuNumber) == 1) {
						PrintWriter script = response.getWriter();
						script.println("<script>");
						...
						PrintWriter script = response.getWriter();
						script.println("<script>");
						script.println("alert('서버에 문제가 발생하였습니다. 다시 시도해주세요 2')");
						script.println("location.href='index.jsp'");
						script.println("</script>");
					}
					...
					if (menuDAO.updateImage(menuTitle, menuContent, menuImageNumber, menuNumber) == 1) {
						PrintWriter script = response.getWriter();
						script.println("<script>");
						script.println("location.href='menuList.jsp'");
						script.println("</script>");
						...
						PrintWriter script = response.getWriter();
						script.println("<script>");
						script.println("alert('서버에 문제가 발생하였습니다. 다시 시도해주세요 3')");
						script.println("location.href='index.jsp'");
						script.println("</script>");
					}
